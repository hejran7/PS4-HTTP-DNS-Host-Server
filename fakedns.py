...
class DNSResponse(object):
    def __init__(self, query):
        ...
        self.flags = DNSFlag(aa=True).pack()  # Authoritative = True
    ...
    def make_packet(self):
        try:
            return self.id + self.flags + self.questions + self.rranswers + self.rrauthority + self.rradditional + self.query + self.pointer + self.type + self.dnsclass + self.ttl + self.length + self.data
...
class RuleEngine2:
    ...
    def __init__(self, rules):
        ...
        for rule in rules:
            ...
            lineno += 1
        if DEBUG:
            print(">> Parsed %d rules" % (len(self.rule_list)))
    ...
    def match(self, query, addr):
        ...
        if addr not in IGNORE:
            ...
            passthru = True
            print(f">> Pass-thru for {addr}: {query.domain.decode()} ({TYPE[query.type]})")
...
def main(interface, port, rule_array, ignore, debug):
    ...
    try:
        ...
        if os.geteuid() != 0 and int(port) < 1024:
            closer("Root privileges may be required to run on udp:{0}".format(port))
        else:
            closer(">> Could not start server -- is another program on udp:{0}?".format(port))
        exit(1)
